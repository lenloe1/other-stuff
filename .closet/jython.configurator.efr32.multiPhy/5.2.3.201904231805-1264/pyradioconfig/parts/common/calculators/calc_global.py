...
class CALC_Global(ICalculator):
    ...
    def buildAdditionalNerioVariables(self, model):
        """
    ...
    def buildAdditionalJumboVariables(self, model):
        """
    ...
    def buildAdditionalNixiVariables(self, model):
        """
    ...
    def buildAdditionalPantherVariables(self, model):
        """
    ...
    def buildAdditionalLynxVariables(self, model):
        """
    ...
    def buildVariables(self, model):
        ...
        self._addModelVariable(model,    'bitrate',  int,                 ModelVariableFormat.DECIMAL, units='bps',  desc='Bit rate after channel coding and before symbol coding.')
        ...
        var = self._addModelVariable(model, 'modulation_type', Enum, ModelVariableFormat.DECIMAL, 'Defines the modulation type.')
        member_data = [
            ['FSK2',  0, 'Frequency Shift Keying on two frequencies'],
            ['FSK4',  1, 'Frequency Shift Keying on four frequencies'],
            ['BPSK',  2, 'Binary Phase Shift Keying: the 2 symbols are represented by 0 or 180 degree phase shifts wrt the carrier'],
            ['DBPSK', 3, 'Differential Binary Phase Shift Keying: the 2 symbols are represented by 0 or 180 degree phase shifts wrt the preceding symbol'],
            ['OOK',   4, 'On Off Keying: the 2 symbols are represented by the presence / absence of the carrier'],
            ['ASK',   5, 'Amplitude Shift Keying: the 2 symbols are represented by two different power levels of the carrier'],
            ['MSK',   6, 'Minimum Shift Keying: Special case of FSK2 where the phase shift in one symbol is +/- 90 degree'],
            ['OQPSK', 7, 'Offset Quadrature Phase Shift Keying: 4 state phase modulation with 0, 90, 180 and 270 degrees wrt the carrier. Only +/-90 degree changes are allowed at any one transition that take place at twice the symbol rate.'],
        ]
        var.var_enum = CreateModelVariableEnum(
            'ModModeEnum',
        ...
        member_data = [
            ['DISABLED',  0, 'Neither AFC nor internal frequency compensation is enabled'],
            ...
            ['AFC_FREE_RUNNING',  4, 'AFC is enabled only and is always on'],
            ['AFC_START_AT_PREAMBLE_FREE_RUNNING',  5, 'AFC is enabled only starting at PREAMBLE_DETECT'],
            ['AFC_LOCK_AT_TIMING_DETECT',  6, 'AFC is enabled only freezing at TIMING_DETECT'],
            ['AFC_LOCK_AT_PREAMBLE_DETECT',  7, 'AFC is enabled only freezing at PREAMBLE_DETECT'],
            ['AFC_LOCK_AT_FRAME_DETECT',  8, 'AFC is enabled only freezing at FRAME_DETECT'],
            ['AFC_START_AT_PREAMBLE_LOCK_AT_FRAME_DETECT',  9, 'AFC is enabled only starting at PREAMBLE_DETECT and freezing at FRAME_DETECT'],
        ]
        var.var_enum = CreateModelVariableEnum(
            'FreqCompModeEnum',
            'TBD.',
        ...
        self.buildAdditionalVariables(model)
        self.buildAdditionalJumboVariables(model)
        self.buildAdditionalNerioVariables(model)
        self.buildAdditionalNixiVariables(model)
        self.buildAdditionalPantherVariables(model)
        self.buildAdditionalLynxVariables(model)

#    def calc_global_hello_world(self, model):
#        print("HELLO from Common Calculator!")
...
class CALC_Radio(ICalculator):
    ...
    def buildVariables(self, model):
        ...
        self._addModelRegister(model, 'RAC.PACTUNECTRL.SGPACTUNETX'    , int, ModelVariableFormat.HEX )
        self._addModelRegister(model, 'RAC.PACTUNECTRL.SGPACTUNERX'    , int, ModelVariableFormat.HEX )
        ...
        if model.part_family.lower() in ["dumbo", "jumbo", "nerio", "nixi"]:
        ...
        elif model.part_family.lower() in ["panther", "lynx"]:
            member_data = [
            ...
            # need a default for unit_test_part, use Series 1 enums
            member_data = [
                ['BW_250KHz', 0, 'PLL loop filter bandwidth is approximately 250 KHz'],
                ['BW_297KHz', 1, 'PLL loop filter bandwidth is approximately 297 KHz'],
                ['BW_354KHz', 2, 'PLL loop filter bandwidth is approximately 354 KHz'],

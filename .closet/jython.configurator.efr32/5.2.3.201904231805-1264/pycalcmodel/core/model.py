...
try:
    from pycalcmodel import __version__
...
class ModelRoot(object):
    ...
    def to_instance_xml_str(self, part_revision, desc, processed, result_code, error_message, profile, phy=None, timestamp=None, phy_group_incl_list=None):
        ...
        outfile = StringIO()  # this is an in memory file
        try:
            retur_val = self._write_instance_to_stream(outfile, part_revision, desc, processed, result_code, error_message, profile, phy, timestamp, phy_group_incl_list)
            if retur_val is not None:
                xml_string = outfile.getvalue()
        finally:
            outfile.close()
    ...
    def _write_instance_to_stream(self, outstream, part_revision, desc, processed, result_code, error_message, profile, phy=None, timestamp=None, phy_group_incl_list=None):
        if self.validate():
            outstream.write("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n")
            if processed:

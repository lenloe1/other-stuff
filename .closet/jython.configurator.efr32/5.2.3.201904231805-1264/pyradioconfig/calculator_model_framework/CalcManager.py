...
class CalcManager(object):
    ...
    def calc_config_phy(self, phy_name, output_dir=None, optional_inputs=None, optional_filename=None):
    ...
    def calc_config(self, phy_name, output_path, optional_inputs=None):
        """Run the calculator and generate the calculated outputs for a single PHY
        ...
        if model_instance.result_code != CalcStatus.Failure.value:
            ...
            self.processed_model_to_cfg(model_instance, output_path, phy_name)
    ...
    def convert_isc_to_cfg(self, isc_filename, xml_filename, cfg_filename):
        ...
        for input in profile.inputs:
            if input._var.var_type is Enum:
                if input.var_value is not None:
                    print("phy.profile_inputs.{0}.value = model.vars.{0}.var_enum.{1}".format(input.var_name, input.var_value.name))
                else:
                    print("phy.profile_inputs.{0}.value = None".format(input.var_name))
            else:
                print("phy.profile_inputs.{0}.value = {1}{2}".format(input.var_name, input.var_value, 'L' if input._var.var_type is long else ''))
    ...
    def phy_has_tag(self, phy_name, tag):
        tag_found_in_tags = False
        modem_instance_model = self.create_modem_model_instance(phy_name)
        phy = modem_instance_model.phy
        if phy.tags is not None:
            if tag in phy.tags:
                tag_found_in_tags = True
        return tag_found_in_tags

    def processed_model_to_cfg(self, model_instance, output_file, phy_name=None):

...
#FIXME: in FDM0 mode make sure osr * bitrate < 10M
    def calc_d0_d1_d2_cfosr_rxbr_value(self, model):
        ...
        if brcalen == 1:
            ...
            rxbrden_list = [31]
            src1_range = [pow(2, (model.vars.src1_bit_width.value - 1) )]
        elif src_disable:
        ...
        for dec0 in [8, 4, 3]:
            ...
            for bwsel in bwsel_list: #[0.263, 0.219, 0.196]:
                ...
                for src1 in src1_range:
                    ...
                    src1ratio = ( pow(2, (model.vars.src1_bit_width.value - 1) ) * 1.0 ) / src1
                    ...
                    if model.part_family.lower() in ["dumbo", "jumbo", "nerio", "nixi"]:
                        if dec1 == 1 and not src_disable and src1 != pow(2, (model.vars.src1_bit_width.value - 1) ):
                            if debug:
                                eliminate0 = 1
                            else:
                                continue
                    ...
                    for dec2 in xrange(dec2_start, dec2_end + 1):
                        ...
                        for rxbrint, rxbrnum, rxbrden in list(itertools.product(rxbrint_list, rxbrnum_list, rxbrden_list)):
                            ...
                            if brcalen or src_disable:
                                src2 = pow(2, (model.vars.src2_bit_width.value - 1) )
                            else:
                                src2 = round((fxo * ( pow(2, (model.vars.src2_bit_width.value - 1) ) * 1.0 ) * src1ratio) / (dec0 * dec1 * dec2 * baudrate * 2 * rxbr))
                            ...
                            if (src2 < ( pow(2, model.vars.src2_bit_width.value - 1 ) * 0.818359375)  or src2 > ( pow(2, model.vars.src2_bit_width.value - 1 ) / 0.819855885) ):
                                if debug:
                                    eliminate3 = 1
                                else:
                                    continue

                            src2ratio = ( pow(2, (model.vars.src2_bit_width.value - 1) ) * 1.0 ) / src2
                            ...
                            if debug:
                                printlist = [cost, bw_error, range_error, rate_error, dec0, dec1, dec2, bwsel, src1, src2, bw, baudrate_real, osr,  eliminate0, eliminate1, eliminate2, eliminate3, eliminate4]

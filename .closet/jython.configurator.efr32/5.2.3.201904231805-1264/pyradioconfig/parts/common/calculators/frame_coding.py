...
class CALC_Whitening(ICalculator):
    ...
    def calc_blockwhitemode(self, model):
        # Arbitrate the proper setting for FRC_FECCTRL_BLOCKWHITEMODEz
        if (model.vars.ber_force_whitening.value == True):
            # Whitening enabled, fec disabled
            fecctrl = 1
        elif ((model.vars.white_poly.value != model.vars.white_poly.var_enum.NONE) and (
            model.vars.fec_en.value != model.vars.fec_en.var_enum.NONE)):
            # Whitening enabled, fec enabled
            fecctrl = 3
        elif (model.vars.white_poly.value != model.vars.white_poly.var_enum.NONE) and (
            model.vars.fec_en.value == model.vars.fec_en.var_enum.NONE):
            # Whitening enabled, fec disabled
            if (model.vars.white_poly.value == model.vars.white_poly.var_enum.PN9_BYTE):
                ...
                fecctrl = 1
            if (model.vars.white_poly.value == model.vars.white_poly.var_enum.Bytewise_XOR_seed_LSB):
                # BYTEWHITE
                fecctrl = 2
        elif model.vars.frame_coding_array_width.value > 0:
            fecctrl = 7  # Use a lookup table
        else:
            fecctrl = 0

        self._reg_write(model.vars.FRC_FECCTRL_BLOCKWHITEMODE,fecctrl)

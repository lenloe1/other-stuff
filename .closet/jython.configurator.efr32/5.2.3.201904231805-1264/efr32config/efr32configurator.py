...
class Efr32Configurator(ChipConfiguratorInterface):
    ...
    def setup(self, _part_family=None, _part_rev=None, _profile_name=None, _adapter_name='rail_api_1.x'):
        """
        ...
        # Create RAIL Adapter
        self._rail_adapter = RAILAdapter(rc_version = self._radio_configurator.version, adapter_name=_adapter_name)
    ...
    def configure (self, **kwargs):
        ...
        if (result_code == CalcStatus.Success.value):
            ...
            self._rail_adapter.populateModel()

            generator = RAILConfig_generator(self._rail_adapter)
            ...
            self.configuration["rail_config.h"] = rail_config_h
            self.configuration["rail_config.c"] = rail_config_c

            # If we have a supported adapter, add the railtest commands for rmr
            if self._rail_adapter.rail_version == 2:
                try:
                    railtest_generator = RailTest_rmrConfigGenerator(self._rail_adapter)
                    rail_railtest_commands = railtest_generator.render(railtest_generator.template_path_railtest)
                    self.configuration["file_names"] = self.configuration["file_names"] + ":rail_test_commands.txt"
                    self.configuration["rail_test_commands.txt"] = rail_railtest_commands
                except Exception as ex:
                    print("Error generating rail_test_commands.txt: {}".format(str(ex)))
                    #No rmr support for multiphy or certain configurations
                    pass

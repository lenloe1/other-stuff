...
class RAILAdapter_SinglePhy(RAILAdapter):
  ...
  def _generateModemConfigEntries(self, phyConfigEntry, model, regs):
    ...
    if len(phyConfigEntry.dynamicSlicerConfigEntries) > 0:
  ...
  def _writeModelToRmDevice(self, radio_model, rm, register_dict):
    ...
    # in the dictionary
    register_dict.pop('SYNTH.FREQ', None)
    register_dict.pop('SYNTH.CHCTRL', None)
    register_dict.pop('SYNTH.CHSP', None)
    register_dict.pop('FRC.BLOCKRAMADDR', None)
    register_dict.pop('FRC.CONVRAMADDR', None)
  ...
  def _generatePhyInfoStructure(self, phyConfigEntry, model):
    ...
    phyConfigEntry.phyInfo.phyInfoMetadata.freqOffsetFactor_fxp.value = outputs.get_output('frequency_offset_factor_fxp').var_value
    phyConfigEntry.phyInfo.phyInfoMetadata.baudPerSymbol.value = outputs.get_output('baud_per_symbol_actual').var_value
    phyConfigEntry.phyInfo.phyInfoMetadata.bitsPerSymbol.value = outputs.get_output('bits_per_symbol_actual').var_value

    phyConfigEntry.phyInfo.phyInfoData.version.value = 0 if self.rail_version == 1 else 3
    phyConfigEntry.phyInfo.phyInfoData.freqOffsetFactor.value = outputs.get_output('frequency_offset_factor').var_value
    phyConfigEntry.phyInfo.phyInfoData.antDivRxAutoConfig.value = antDivConfiguration
    phyConfigEntry.phyInfo.phyInfoData.src1Denominator.value = outputs.get_output('src1_calcDenominator').var_value
    phyConfigEntry.phyInfo.phyInfoData.src2Denominator.value = outputs.get_output('src2_calcDenominator').var_value
    phyConfigEntry.phyInfo.phyInfoData.txBaudRate.value = outputs.get_output('tx_baud_rate_actual').var_value
    phyConfigEntry.phyInfo.phyInfoData.rateInfo.value = outputs.get_output('tx_baud_rate_actual').var_value
  ...
  def populateModel(self):
    ...
    for phy in sorted(self.instanceDict):
      ...
      self.partFamily = model.part_family

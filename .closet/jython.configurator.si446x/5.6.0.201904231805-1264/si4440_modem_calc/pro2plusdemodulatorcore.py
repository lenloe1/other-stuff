...
class Pro2PlusDemodulatorCore(Pro2DemodulatorCore):
    ...
    def _init_preamble_pattern(self, modem_calc) :
        ...
        if (modem_calc.inputs.API_pm_pattern in [0, 4, 100, 110, 120]) :  # 1010 PM pattern: Min pm length = 32 or 40 bits
            ...
            self.fields.preath   = 0       # skip pmdet detection
...
# 2GFSK  pre/post cursor compensation is always disabed
    ...
    def _init_signal_dsa_mode  (self, modem_calc):
        ...
        if (modem_calc.inputs.API_pm_pattern in [4, 100, 110, 120, 1000]):        
            self.fields.signal_dsa_mode   = 1
        else:
    ...
    def _pre_calc_lut(self, modem_calc):
        ...
        self.fields.large_freq_err = (self.fields.tx_rx_err > 16*modem_calc.inputs.API_Rsymb*0.75 and
                                 self.fields.nonstdpk == 0 and
                                 en2fsk(modem_calc))
        ...
        if(modem_calc.inputs.API_BER_mode==0):  
            if(Fdev_error_hi>15 or Fdev_error_lo>15):
    ...
    def _calc_step_6(self, modem_calc):
        ...
        if(modem_calc.inputs.API_ant_div==1 or modem_calc.inputs.API_pm_pattern == 1000):
        ...
        if (self.fields.oneshot_afc==1 and (modem_calc.inputs.API_ant_div==1 or modem_calc.inputs.API_dsa_mode == 3)):
    ...
    def _calc_step_12(self, modem_calc):
        ...
        if (self.fields.nonstdpk==1 and self.fields.detector==2 ) : # MA nonstdpk
            ...
            self.afc_gain_abs_pro2 =  self.afc_gain_cal/2**self.afc_adj
        # PRO2+'s AFC gain
        if (self.fields.oneshot_afc == 1):
            if(modem_calc.inputs.API_pm_len >= 16 and modem_calc.inputs.API_pm_len <32):
                    self.afc_gain_abs = self.afc_gain_abs_pro2/4
            elif (modem_calc.inputs.API_pm_len >= 32 or modem_calc.inputs.API_ant_div==1):
    ...
    def _calc_step_17(self, modem_calc):
        ...
        if (engfsk(modem_calc.inputs.API_modulation_type)) :
            ...
            #TODO: why is the last 26 a 2 in pro2 version?
    ...
    def _calc_step_20(self, modem_calc):
        ...
        if (modem_calc.inputs.API_raw_demod==1 or
            self.fields.signal_dsa_mode   == 1) :
    ...
    def _calculate(self, modem_calc):
        ...
        self.fields.spike_det_thd = max(self.min_spike_det_thd,min(127, self.raw_diff*0.8))

        if (modem_calc.inputs.API_dsa_mode == 0):
        ...
        if (en4gfsk(modem_calc.inputs.API_modulation_type) and 
            self.fields.thd4gfsk_comp_en == 1):
            self.fields.bw_peak = self.fields.thd4gfsk_comp_en*2**7 + min(127, self.fields.thd4gfsk_comp_ratio)
